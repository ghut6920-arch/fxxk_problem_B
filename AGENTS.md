...
In-repository operating roles are **Technical Lead**, **Executor**, and **Red Team**. Strategist is an external advisor only (`docs/roles/strategist.md`); it does not operate the repository.
...
| `STATUS.md` | Verified current state, risks, blockers, decision references | Technical Lead; model-family state cites recorded external-advisor notes when applicable |
| `NEXT_ACTION.md` | One Primary Integration Goal and acceptance criteria | Technical Lead; changes to competition objective or model family escalate to the user |
| `DECISIONS.md` | Major decisions and their evidence, scope, effects, reopen conditions | Technical Lead records them; model-level decisions arrive as external-advisor notes |
| `ASSUMPTIONS.md` | Assumptions, interpretations, unresolved conflicts | Technical Lead maintains the file; modeling interpretations cite recorded advisor notes; facts cite rule audits |
| `modeling/`, later `MODEL_SPEC.md` | Model design, approved selection, formal implementation contract | Technical Lead coordinates files; selection remains an explicit recorded decision, not an in-repo Strategist role |
...
Current strategic decisions, selection, and model contract must agree. New evidence may challenge a decision but does not silently replace it. Supersession and downstream model-family changes must be recorded explicitly (from an external advisor note and/or user authorization). STATUS, NEXT_ACTION, and Spikes cannot override official rules or the active contract.
...
- Strategic Review: `KEEP`, `MODIFY`, or `REJECT` for a specified mathematical approach, with scope and reopen conditions. KEEP does not replace technical verification. Strategic Review is produced by an **external advisor** (formerly the in-repo Strategist) and recorded by Technical Lead; the advisor does not operate the repository.
...
- Routine local work needs Technical Review. Selection and model-family changes need a recorded Strategic Review from the external advisor (and user authorization when the change is project-level). Stage-level proposals, formal deliverables, and key claims receive Red Team checks; every small edit need not pass all three.
...
- Unresolved CRITICAL findings block affected formal promotion. Technical Lead arranges and verifies technical repairs; model-family adjudication uses a recorded external Strategic Review; Red Team appends recheck evidence to original findings.
- A CRITICAL finding is closed only through a reasoned Strategic Review (external advisor note recorded by Technical Lead, or explicit user authorization) citing the affected version, repair or counter-evidence, and Red Team recheck record, and addressing each remaining objection. Closure requires evidence that the finding is repaired, refuted, or no longer applies to the explicitly limited target. KEEP or risk acceptance alone cannot close a still-valid CRITICAL finding.
- Red Team retains its original finding and any dissent; its agreement is not an additional approval requirement or indefinite veto. The recorded evidence-based Strategic Review determines closure. New material evidence may reopen the finding under the recorded reopen conditions.
- Formal promotion: a recorded Strategic Review (external advisor and/or user) explicitly approves the named results/version for inclusion in the formal solution; Technical Lead checks that approval, applicable reviews, reproducibility, version consistency, and closure requirements; Executor writes only those approved artifacts to results/formal/ under a WI issued after that check. Retain approval and check references in the WI. None of these steps substitutes for another.
...
Changes to core assumptions, objective, constraint meaning, model family, formal selection, or two or more downstream modeling stages require a recorded external Strategic Review and, for project-goal boundaries, the user. Evidence conflicting with the model mechanism follows the same route. Engineering complexity alone is not a strategic change.
...
- `main`: reviewed, internally consistent integration state (Technical Lead).
- `feat/WI-NNN-*` or `feat/NA-NNN-*`: Executor assigned work.
- `experiment/EXP-NNN-*`: uncertain candidate experiments.
- `review/RV-NNN-*`: independent review and Red Team work.
- Historical `design/NA-NNN-*` branches may exist as frozen evidence; they are not a live in-repo Strategist worktree.

Long-lived worktrees are Technical Lead (`B题` / `main`), Executor (`B题-executor`), and Red Team (`B题-redteam`). Branches describe work, not Agents or members; no `codex/*`, `deepcode/*`, `grok/*`, or member-named equivalents. Coordinate overlapping core-file edits before writing. Red Team must not directly modify formal code in the main worktree. Merge only after the applicable review.
