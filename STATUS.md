...
Official inputs and prior plan/design remain as recorded below. WI-014 evaluator/fixtures are on `origin/main`. WI-015 candidate `e4ee8fe` and WI-016 run `761c0f7` are technically accepted (TR-014/015/016) **off** `main`. WI-016 conclusion is `P1A_PROPERTIES_PASS` as a D-004/SR-002 same-model finite-fixture check only. Isolation residual remains. Not selection. Candidate/run not merged, not pushed.
...
- WI-016 is accepted as finite-fixture internal consistency. Next is user choice: retrieve run report onto `main`, optional RT4-F2/F3 repair, or later P1-B only after simulator-origin audit. Do not open C1/C2/P4/P5, consume a formal slot, close `RT-002`, or write independent-evaluation claims. Independent challenge of run `761c0f7` remains absent.
