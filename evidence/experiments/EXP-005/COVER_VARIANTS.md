...
`tests/candidate/test_variants.py` (**32 tests**, WI-027 revision): tag table and Q3/Q4 arm matrix;
unknown tags refused; **BASE equivalence** (clear centres, clear plan and budget equal the frozen
objects, and a default `C0Runner` uses the frozen path); CLEAR150 set/order — the 30 m wrap is
asserted **at index 74** between `(1490, -15)` and `(1490, 15)`, all other 148 steps are exactly 20 m,
and the same holds at `theta in {0, 45, 90, 123.456, -37, 0.5}` deg; tile cover `sqrt(325)+1 < 20`;
submitted path 3288 m; SCAN49 membership, a discriminating lattice validator that **rejects the 5x5
substitute**, and a per-point guard that any of the **28 exterior points** missing is detected;
...
Canonical digest (WI-027): the **committed blob** `5390c53:evidence/experiments/EXP-005/worlds.json`
hashes to

```
f00b1718f64de2dae3bff38187805f6e75c2697cf4cad4d80362f588b0e573f6      (len 26082, LF)
```

This is the digest to verify against; it is reproduced by
`git show 5390c53:evidence/experiments/EXP-005/worlds.json | sha256sum`.

> **Withdrawn digest.** The earlier text recorded
> `c4911cea0841983a39a0c57a991acaa77b5ae228dc47aa4b07cfd4fefb8c5d99`, which is the SHA-256 of the
> **working-tree copy after the Windows checkout expanded the file to CRLF** (len 27723), not of the
> committed object. Both digests cover the same worlds — the two byte strings parse to **identical
> JSON** (verified: 12 worlds, every coordinate and radius equal) — but only the committed-blob digest
> is reproducible by a reviewer, so `c4911cea…` is **withdrawn** and must not be cited. No world
> coordinate changed, and the 12 worlds were **not** regenerated.
...
## 10. WI-027 — RT7-F1/F2 remediation

**Conclusion: `HASH_BOUND`**

This section supersedes nothing in sections 1-9 except the two items below; the WI-023 result
(`COVER_VARIANTS_READY`), the 36-track numbers and the recommendation are unchanged. The 12 worlds
were **not** regenerated, the 36 tracks were **not** re-run, and `src/` was not edited.

### RT7-F1 — worlds hash provenance (fixed)

The canonical digest is now the **committed blob**:
...
`c4911cea0841983a39a0c57a991acaa77b5ae228dc47aa4b07cfd4fefb8c5d99` is **withdrawn** (see the box in
section 6). Root cause: the recorded value was computed over the **working-tree bytes** after the
Windows checkout expanded LF to CRLF (27723 bytes), while a reviewer necessarily recomputes over the
**committed object** (26082 bytes) — so the provenance chain appeared broken even though no content
differed. Verified equivalence: the `git show …` bytes and the working-tree bytes parse to
**identical JSON** (12 worlds, all coordinates and radii equal), so no coordinate changed and no
regeneration was warranted. No source coordinates were touched by this WI.

### RT7-F2 — misleading SCAN49 unit test (fixed)

`test_the_exterior_ring_is_not_needed_for_in_disk_coverage` (WI-023) computed the covering radius of
the 5x5 inner lattice, found that it also covers the target disk, and framed that as "the exterior
points are conformance, not coverage". The remark is true as a coverage statement but it read as an
endorsement of a 25-point substitute for the prescribed `P_4'`, so the test is **deleted** and
replaced by:

* `assert_is_scan49_lattice` — a discriminating validator that accepts only the complete 49-point
  `{700(i, j) : i, j = -3..3}` set and explicitly rejects the 5x5 lattice.
* `test_inner_5x5_lattice_cannot_pass_as_scan49` — the real generator passes; the 5x5 set is rejected.
* `test_every_one_of_the_28_exterior_points_is_required` — dropping **any single one** of the 28
  points outside the radius-1800 region (the 24-point outer ring plus the four diagonal
  `(+/-1400, +/-1400)` points at norm 1979.9 m) is detected: 28 cases.
* `test_a_5x5_lattice_would_change_the_recorded_scan_counts` — the plan's own counts reject a
  substitute: 49 points / 980 measures versus 25 points / 500 measures for the 5x5.
* The neighbouring exterior test now states both counts explicitly (24 outer ring vs 28 outside the
  target region) so the two notions cannot be conflated again.

**Non-vacuity check (mutation, not a committed test):** monkeypatching `variants.scan49_points` to
return the 5x5 lattice makes **9 tests fail**, including both anti-masquerade tests, so the guard
cannot silently pass a substituted lattice.

### Required command

`PYTHONPATH=src python -m unittest tests.candidate.test_variants -q` → **OK, 32 tests** (was 30).
